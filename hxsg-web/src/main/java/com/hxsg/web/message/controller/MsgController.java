package com.hxsg.web.message.controller;
...
public class MsgController {
    ...
    public String appsxmsg(Model model,HttpServletRequest request,HttpServletResponse response,HttpSession session)throws Exception {
        List<roleMessage> li=rmm.getMsg();
        if(li.size()>0&&li!=null){
           com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("rmsg",li,request, response);

        }else{
            com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("code",1,request, response);
        }
        return null;
