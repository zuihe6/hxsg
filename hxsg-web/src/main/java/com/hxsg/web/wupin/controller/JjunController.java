    ...
    public synchronized String appsyjjl(HttpServletRequest request,HttpServletResponse response,@RequestParam(value = "name", required = false) String name,@RequestParam(value = "id", required = false) Integer id, Model model, HttpSession session) throws Exception {
        ...
        if(roleid>0){
            ...
            if(jj.getType().equals("书")){
                ...
                //model.addAttribute("lis", li);
                if(li.size()<=0){
                    com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("li",0,request,response );
                }else{
                    int se=li.size();
                    li.add(se,jj.getJjname());
                    li.add(se+1,id);

                    com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("li",li,request,response);
            ...
            if(jj.getType().equals("令")){
                ...
                if (a > 0) {
                    ...
                    if (rand > gailv) {//百分之40的概率产生
                        ...
                        com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("fjname", rf.getFujiangname(),request,response);
                    } else {
                        com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("fjfail",1,request,response);
