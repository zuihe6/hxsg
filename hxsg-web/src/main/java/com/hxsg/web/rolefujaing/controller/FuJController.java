...
import com.hxsg.core.po.jiangjun;
...
public class FuJController {
    ...
    public String appfjqk(@RequestParam(value = "id", required = false) Integer id, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (id != null && id != 0) {
            FuJiang fj = fjm.selectByPrimaryKey(id);
            com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("rf", fj, request, response);
        }
    ...
    public String appckds(@RequestParam(value = "id", required = false) Integer id, HttpServletResponse response, HttpServletRequest request) throws Exception {
        if (id != null && id != 0) {
            RoleFujiang rf = rfm.selectByPrimaryKey(id);
            com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("rf", rf, request, response);
        ...
        if (r > 0) {
            ...
            if (rolemoney >= rymoney) {//如果携带银两大于培养消耗的金
                ...
                if (type.equals("成长")) {
                    ...
                    rf.setChengzhang(ft);
                    rfm.updateByPrimaryKeySelective(rf);
                    com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("num", ft, request, response);
                ...
                com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("msg", "你金额不足，无法培养。", request, response);
    ...
    public String appfpsx(HttpServletRequest request, RoleFujiang rfg, HttpServletResponse response, HttpSession session) throws Exception {
        if (rfg != null) {
            ...
            rf.setSududs(rfg.getSududs());
            rfm.updateByPrimaryKeySelective(rf);
            com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("msg", "恭喜你分配成功！", request, response);
        ...
        if (total >= num) {
            ...
            if (a > 0) {
                ...
                if (jj.getJjname().equals("副将经验书") && rf.getLeve() <= 11) {
                    ...
                    rf.setSududs(11);
                    rfm.updateByPrimaryKeySelective(rf);
                    com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("sjmsg", "恭喜你副将升到11级", request, response);
                ...
                if (rf.getJingyan() >= 0) {
                    ...
                    if (rfg.getSjjinyan() <= 0) {//升了一级之后的属性
                        ...
                        com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("sjmsg", "副将获得" + 60 * jy * jy * num + "经验，恭喜你副将升到" + (jy + 1) + "级", request, response);
                    ...
                    com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("msg", "副将获得" + 60 * jy * jy * num + "经验", request, response);
            ...
            com.hxsg.web.CommonUtil.CommonUtilAjax.sendAjaxList("msg", "你身上携带的数量不足，请重新输入。", request, response);
